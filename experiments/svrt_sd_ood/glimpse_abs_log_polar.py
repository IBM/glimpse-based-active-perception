...
class Experiment(BaseExperiment):
    def __init__(
        ...
        super().__init__(
            ...
            os.path.join(
                LOGDIR_PATH_PREFIX,
                "glimpse_models_logs/svrt_sd_ood/glimpse_abs_log_polar"))
        ...
        batch_size = 100
        scales = []
        sensor = LogPolarSensor(glimpse_size, radius=polar_radius)
        ...
        self.model = GlimpseBasedModel(
            ...
            SaliencyMapBasedGlimpsing(
                image_size=128,
                ior_mask_size=mask_size, glimpse_size_to_mask_size_ratio=1,
        ...
        self.train_ds = DataLoader(
            SVRTFull(
                os.path.join(DATA_PATH, 'data'), True, task_idx=1, num_samples=28000,
                rgb_format=True,
            ),
            batch_size,
            shuffle=True)
        self.test_ds = DataLoader(
            SVRTFull(os.path.join(DATA_PATH, 'data'), False, task_idx=1, num_samples=2000, rgb_format=True),
            batch_size)
        ...
        for ds_name in dataset_names:
            ds = DataLoader(
                OOD_SVRTFull(ds_name, root=os.path.join(DATA_PATH, 'data'), rgb_format=True),
                batch_size)
